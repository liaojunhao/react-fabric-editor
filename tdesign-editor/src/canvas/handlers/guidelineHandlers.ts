...
class GuidelineHandlers {
  ...
  aligningLineOffset = 2;
  aligningLineMargin = 4;
  aligningLineWidth = 1;
  aligningLineColor = 'rgb(255, 73, 255)';
  ...
  constructor(private handlers: Handlers) {
    this.handlers.canvas.on('before:render', this.beforeRender.bind(this));
    this.handlers.canvas.on('after:render', this.afterRender.bind(this));

    this.ctx = this.handlers.canvas.getSelectionContext();
    this.aligningLineOffset = 5;
    this.aligningLineMargin = 4;
    this.aligningLineWidth = 1;
    this.aligningLineColor = 'rgb(255, 0, 0)';
    this.viewportTransform = this.handlers.canvas.viewportTransform;
    this.zoom = this.handlers.canvas.getZoom();
    this.verticalLines = [];
  ...
  movingGuidelines(target) {
    ...
    const canvasObjects = this.handlers.canvas.getObjects();
    ...
    const { _currentTransform: transform } = this.handlers.canvas;
    if (!transform) return;
    ...
    for (let i = canvasObjects.length; i--; ) {
      ...
      if (this.isInRange(objectLeft, activeObjectLeft)) {
        ...
        target.setPositionByOrigin(new Point(objectLeft, activeObjectTop), 'center', 'center');
      }
      ...
      if (this.isInRange(objectLeft + objectWidth / 2, activeObjectLeft + activeObjectWidth / 2)) {
        ...
        if (canvasObjects[i].id === 'workarea') {
          ...
          //@ts-ignore
          if (canvasObjects[i].layout === 'fullscreen') {
            x = this.handlers.canvas.getWidth();
          }
          this.verticalLines.push({
            x,
            y1,
      ...
      if (this.isInRange(objectTop, activeObjectTop)) {
        ...
        target.setPositionByOrigin(new Point(activeObjectLeft, objectTop), 'center', 'center');
      }
      ...
      if (this.isInRange(objectTop + objectHeight / 2, activeObjectTop + activeObjectHeight / 2)) {
        ...
        if (canvasObjects[i].id === 'workarea') {
          ...
          //@ts-ignore
          if (canvasObjects[i].layout === 'fullscreen') {
            y = this.handlers.canvas.getHeight();
          }
          this.horizontalLines.push({
            y,
            x1,
  ...
  beforeRender() {
    this.handlers.canvas.clearContext(this.handlers.guidelineHandlers.ctx);
  }
...
export default GuidelineHandlers;
