...
async function psdToJson(psdFile) {
  ...
  // 设置背景
  const { clipPath, workarea } = getClipPath(psdFile);
  json.clipPath = clipPath;
  json.objects.push(workarea);
    ...
    return childrensFilter.map(async (item) => {
      ...
      const typeList = [
        ...
        },
        {
          ...
          attr: ['text'],
          async customTransform(item) {
            const currentObj = item.textProperties.EngineDict.StyleRun.RunArray[0].StyleSheet.StyleSheetData;
            // console.log('currentObj ---> ', currentObj.FontSize);
            // TODO:
            // const { FontSize, FillColor, Tracking } =
            //   item.textProperties.EngineDict.StyleRun.RunArray[0].StyleSheet.StyleSheetData;
            return {
              charSpacing: currentObj.Tracking,
              fontFamily: '站酷快乐体',
              fill: 'rgb(0, 0, 0)',
              fontSize: currentObj.FontSize || 12,
            };
          },
        },
      ];
...
function getColor(arr?: [number, number, number, number]) {
  const [, r, g, b] = arr;
